...
### 联系方式解锁相关风险（MVP → Pro 阶段需关注）

> MVP 阶段产品决策：被申请方点击"接受"后，申请方可看到对方完整电话号码。此机制引入以下风险，需在 Pro 阶段逐步解决。

#### 风险1：假账号批量钓取号码（高风险）

**场景**：恶意用户注册假资料，向大量真实用户发送申请。一旦对方"接受"，就能获取真实电话号码，用于电信诈骗或骚扰。老年用户群体对这类风险的警惕性较低。

**Pro 阶段防护方案**：
- 只有完成认证（至少标准认证）的用户，其申请被接受后才能看到对方完整号码
- 未认证用户被接受后，只能通过平台内聊天联系，不暴露电话号码
- 在认证系统的文案中将此作为认证价值的卖点："认证后才能查看对方联系方式"

#### 风险2：反向钓鱼——用假号码换真号码（中风险）

**场景**：有人用假电话号码（如随便填一个 13800001111）发布资料，然后接受别人的申请。对方看到的是假号码打不通，但自己已经拿到了对方的真实号码。

**Pro 阶段防护方案**：
- 标准认证要求手机号实名验证（发送短信验证码），确保号码真实
- 解锁联系方式时优先展示认证验证过的号码，而非用户自填的号码
- MVP 阶段可接受此风险（用户量极小，且留言中通常已包含联系方式）

#### 风险3：接受后无法反悔（低风险）

**场景**：用户不小心点了"接受"，或接受后发现对方不合适，想撤回。但电话号码已经被对方看到了，即使加"撤回接受"功能，对方也可能已经记下号码。

**Pro 阶段应对方案**：
- 技术上可以增加"撤回接受"功能（状态从 accepted 改回 viewed），撤回后对方详情页不再显示完整号码
- 需要在 UI 中提示用户："撤回后对方将无法再查看您的号码，但此前已查看过的信息无法收回"
- 这是所有此类平台的通用风险，无法完全消除，重点是在"接受"前的确认对话框中做好知情提醒（已实现）

#### 风险4：申请轰炸（中风险）

**场景**：某用户每天向平台上所有异性资料发申请，造成其他用户收到大量无意义的申请消息，影响体验。

**Pro 阶段防护方案**：
- 每日申请上限：未认证用户每天最多发 5 个申请，标准认证 10 个，高级认证 20 个
- 申请冷却期：向同一用户被拒绝后，30天内不能再次申请
- 被多次拒绝的用户触发预警：如果一个用户被拒绝超过 10 次，自动进入人工审核队列

#### 实施优先级

| 风险 | MVP阶段 | Pro第一阶段 | Pro第二阶段 |
|------|---------|------------|------------|
| 假账号钓号码 | 接受风险（用户量小） | 认证用户才能解锁号码 | AI风控检测异常账号 |
| 反向假号码 | 接受风险 | 手机号验证码认证 | — |
| 接受后反悔 | 知情提醒已实现 | 增加"撤回接受"功能 | — |
| 申请轰炸 | 已有重复申请检查 | 每日申请上限 | 智能风控 + 举报机制 |
...
| 来源 | 描述 | 实现方式 |
|------|------|----------|
| **认证费用** | 平台主要收入来源 | 分级认证系统（¥99-¥699/年） |
| **增值服务** | 辅助收入 | 置顶推荐、数据报告等 |
| **广告** | 长期考虑 | 平台广告位（未来考虑） |

### 定价策略
- **标准认证（¥99/年）**：基础认证，覆盖核心需求
- **高级认证（¥299/年）**：提供视频认证、置顶推荐等增强功能
- **尊享认证（¥699/年）**：VIP客服、数据分析报告等高端服务

### 转化率提升策略

初期认证转化率预计在 **1-5%** 范围内（参考国内SaaS产品平均水平）。为提升转化率，需要实施以下策略：

1. **排序优先权** - 认证用户的资料在首页获得更高排序权重
2. **曝光优势** - 认证用户的资料获得更多推荐和展示机会
3. **筛选权限** - 认证用户可设置"仅接收认证用户的申请"，提高匹配质量
4. **信任标签** - 认证用户显示蓝V标签和认证等级，建立信任
5. **运营活动** - 定期推出"认证用户优惠月"等营销活动
6. **教育引导** - 在关键路径上引导用户了解认证的价值

### 财务规划原则

- **初期目标**：建立稳定的用户基数和认证体系，而非追求高转化率
- **中期目标**：通过产品优化和运营推动，逐步提升认证转化率至 5-10%
- **长期目标**：打造认证用户社区，形成良性循环
- **风险管理**：不依赖单一收入来源，探索多元化变现方式
